...
columns_needed = ['DIST_SGNL', 'NAVIGATIONAL_STATUS', 'LONGITUDE', 'LATITUDE', 'E_CO2_kg']
...
for filename in tqdm(files):
    if "hourly" in filename and filename.endswith(".csv"):
        ...
        signals_df = pd.read_csv(file_path, low_memory=False)
        signals_df = signals_df[(signals_df['DIST_SGNL'] < 3) | (signals_df['NAVIGATIONAL_STATUS'].isin(['at_berth', 'anchored']))]
        if len(signals_df)==0:
            continue
        ...
        joined=joined[joined['Distance_to_Port']<8]
        joined=joined[['E_CO2_kg','PORT_NAME_right']]
        aggregated_data = joined.groupby('PORT_NAME_right')['E_CO2_kg'].sum().reset_index()
        
        for index, row in aggregated_data.iterrows():
            port_name = row['PORT_NAME_right']
